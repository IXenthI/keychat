...
- A chat box appears at the bottom so you can talk in your primary channel from the dock, with an **emote picker** (😀 button) — searchable; click to insert. It has ⭐ **Favorites** (hover any emote and click its star) and 🕐 **Frequently used** rows pinned at the top, then your channel's emotes, then a labeled section per source so nothing is bunched together. Favorites and usage counts are remembered in your browser.
- The emote picker also includes your **Twitch emotes** (global + sub emotes from every channel you're subscribed to) once you grant the `user:read:emotes` permission (`/logout` and sign in again) — each subscribed channel gets its own labeled section
